...
require_once($CFG->dirroot . '/mod/quiz/report/allocatedgrading/allocatedgradingsettings_form.php');
 ...
 */
class quiz_allocatedgrading_report extends report_base {
    ...
    /** @var quiz_allocatedgrading_renderer Renderer of Quiz Grading. */
    protected $renderer;
    ...
    /**
     * Display the allocated grading report.
     *
    ...
    public function display($quiz, $cm, $course) {
        ...
        $pagesize = optional_param('pagesize',
                get_user_preferences('quiz_allocatedgrading_pagesize', self::DEFAULT_PAGE_SIZE),
                PARAM_INT);
        $page = optional_param('page', 0, PARAM_INT);
        $order = optional_param('order',
                get_user_preferences('quiz_allocatedgrading_order', self::DEFAULT_ORDER),
                PARAM_ALPHAEXT);
        ...
        require_capability('quiz/allocatedgrading:grade', $this->context);

        if (!has_capability('quiz/allocatedgrading:assigngrader', $this->context) &&
                !$this->user_can_view_report()) {
            $url = new \moodle_url('/');
            redirect($url, get_string('gradingnotallowed', 'quiz_allocatedgrading'),
                messagetype: \core\output\notification::NOTIFY_ERROR);
        }

        $shownames = has_capability('quiz/allocatedgrading:viewstudentnames', $this->context);
        // Whether the current user can see custom user fields.
        $showcustomfields = has_capability('quiz/allocatedgrading:viewidnumber', $this->context);
        ...
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'quiz_allocatedgrading');
        }
        ...
        if (!$hasquestions) {
            $this->print_header_and_tabs($cm, $course, $quiz, 'allocatedgrading');
        ...
        if (!$counts || $counts->$grade == 0) {
            redirect($this->list_questions_url(), get_string('alldoneredirecting', 'quiz_allocatedgrading'));
        }
    ...
    protected function base_url() {
        return new moodle_url('/mod/quiz/report.php',
                ['id' => $this->cm->id, 'mode' => 'allocatedgrading']);
    }
    ...
    protected function display_index($includeauto) {
        ...
        $this->print_header_and_tabs($this->cm, $this->course, $this->quiz, 'allocatedgrading');
        ...
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'quiz_allocatedgrading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'quiz_allocatedgrading');
        }
        echo $this->renderer->render_display_index_heading($linktext, $this->list_questions_url(!$includeauto));
        $data = [];
        $header = [];

        $header[] = get_string('qno', 'quiz_allocatedgrading');
        $header[] = get_string('qtypeveryshort', 'question');
        $header[] = get_string('questionname', 'quiz_allocatedgrading');
        if (has_capability('quiz/allocatedgrading:assigngrader', $this->context)) {
            $header[] = get_string('assignedgrader', 'quiz_allocatedgrading');
        }
        $header[] = get_string('tograde', 'quiz_allocatedgrading');
        $header[] = get_string('alreadygraded', 'quiz_allocatedgrading');
        if ($includeauto) {
            $header[] = get_string('automaticallygraded', 'quiz_allocatedgrading');
        }
        $header[] = get_string('total', 'quiz_allocatedgrading');
        ...
        foreach ($statecounts as $counts) {
            ...
            if (!has_capability('quiz/allocatedgrading:assigngrader', $this->context) &&
                !$this->user_can_grade_question($this->questions[$counts->slot]->slot)) {
                continue;
            }
            ...
            if (has_capability('quiz/allocatedgrading:assigngrader', $this->context)) {
                $allocatedusers = $DB->get_fieldset("quiz_allocatedgrading", 'userid', [
                    "quizid" => $this->quiz->id,
                    "slotid" => $counts->slot,
                ]);
                ...
                $enrolledusers = [];
                foreach (get_enrolled_users($this->context, groupids: $groupids) as $user) {
                    // Concatenate firstname and lastname.
                    $enrolledusers[$user->id] = "{$user->firstname} {$user->lastname}";
                }

                $editable = new \quiz_allocatedgrading\output\user_allocatedgrading_editable($this->quiz->id,
                                                                                             $this->questions[$counts->slot]->slot,
                                                                                             $allocatedusers,
                                                                                             $enrolledusers,
                                                                                             $this->context
                );
                $row[] = $this->renderer->render_from_template('core/inplace_editable', $editable->export_for_template($OUTPUT));
            }

            $row[] = $this->format_count_for_table($counts, 'needsgrading', 'gradeverb', 'moodle');

            $row[] = $this->format_count_for_table($counts, 'manuallygraded', 'updategrade', 'quiz_allocatedgrading');

            if ($includeauto) {
                $row[] = $this->format_count_for_table($counts, 'autograded', 'updategrade', 'quiz_allocatedgrading');
            }

            $row[] = $this->format_count_for_table($counts, 'all', 'gradeall', 'quiz_allocatedgrading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new quiz_allocatedgrading_settings_form($hidden, $counts, $shownames, $showcustomfields, $this->context);
        ...
        if ($mform->is_submitted()) {
            if ($mform->is_validated()) {
                ...
                set_user_preference('quiz_allocatedgrading_pagesize', $pagesize);
                set_user_preference('quiz_allocatedgrading_order', $order);
                ...
                $pagesize = get_user_preferences('quiz_allocatedgrading_pagesize', self::DEFAULT_PAGE_SIZE);
            }
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        $params = [];
        $userfieldsapi = \core_user\fields::for_identity($this->context)->with_name();
        $userfieldssql = $userfieldsapi->get_sql('u', true, '', 'userid');
    ...
    public function print_header_and_tabs($cm, $course, $quiz, $reportmode = 'overview') {
        global $PAGE;
        $this->renderer = $PAGE->get_renderer('quiz_allocatedgrading');
    ...
    protected function get_question_heading(stdClass $attempt, bool $shownames, bool $showcustomfields): string {
        ...
        if ($shownames && $showcustomfields) {
            return get_string('gradingattemptwithcustomfields', 'quiz_allocatedgrading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'quiz_allocatedgrading', $a);
    ...
    /**
     * Check if user assigned to grade any question.
     *
     * @return void
     */
    private function user_can_view_report(): bool {
        global $DB, $USER;
        return $DB->record_exists('quiz_allocatedgrading', [
            "quizid" => $this->quiz->id,
            "userid" => $USER->id,
        ]);
    }
    ...
    /**
     * Check if a user assigned for grading a question.
     *
     * @param int $questionslotid question slot id.
     * @return bool
     */
    private function user_can_grade_question(int $questionslotid): bool {
        global $DB, $USER;
        return $DB->record_exists('quiz_allocatedgrading', [
            "quizid" => $this->quiz->id,
            "userid" => $USER->id,
