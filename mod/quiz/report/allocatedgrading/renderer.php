...
/**
 * Defines the renderer for the quiz_allocatedgrading module.
 *
...
/**
 * The renderer for the quiz_allocatedgrading module.
 * @author     Vladislovas Kidanovas
 * @copyright  2018 Huong Nguyen <huongnv13@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_allocatedgrading_renderer extends plugin_renderer_base {
    ...
    public function render_quiz_no_grade_question_notification() {
        return $this->notification(get_string('nothingfound', 'quiz_allocatedgrading'), 'info', false);
    }
    ...
    public function render_display_index_heading($linktext, $listquestionurl) {
        ...
        $output .= $this->heading(get_string('questionsthatneedgrading', 'quiz_allocatedgrading'), 3);
     ...
     * @param quiz_allocatedgrading_settings_form $filterform Question filter form.
    ...
    public function render_grading_interface($questioninfo, $listquestionsurl, $filterform, $paginginfo, $pagingbar, $formaction,
        ...
        $output .= $this->heading(get_string('gradingquestionx', 'quiz_allocatedgrading', $questioninfo), 3);

        $output .= html_writer::tag('p', html_writer::link($listquestionsurl,
                get_string('backtothelistofquestions', 'quiz_allocatedgrading')),
                ['class' => 'mdl-align']);

        $output .= $filterform->render();

        $output .= $this->heading(get_string('gradingattemptsxtoyofz', 'quiz_allocatedgrading', $paginginfo), 3);
        ...
        $output .= html_writer::tag('div', html_writer::empty_tag('input', [
                ...
                'value' => get_string('saveandnext', 'quiz_allocatedgrading'),
        ]), ['class' => 'mdl-align']);
        ...
        $this->page->requires->js_call_amd('core_form/changechecker', 'watchFormById', ['allocatedmanualgradingform']);
